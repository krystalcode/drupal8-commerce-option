 ...
 */
class ProductOptionForm extends BundleEntityFormBase {

  /**
   * The option field manager.
   *
   * @var \Drupal\commerce_option\ProductOptionFieldManagerInterface
   */
  protected $optionFieldManager;
  ...
  /**
   * Constructs a new ProductOptionForm object.
   *
   * @param \Drupal\commerce_option\ProductOptionFieldManagerInterface $option_field_manager
   *   The option field manager.
   * @param \Drupal\commerce\InlineFormManager $inline_form_manager
   *   The inline form manager.
   */
  public function __construct(ProductOptionFieldManagerInterface $option_field_manager, InlineFormManager $inline_form_manager) {
    $this->optionFieldManager = $option_field_manager;
  ...
  public static function create(ContainerInterface $container) {
    return new static(
      $container->get('commerce_option.option_field_manager'),
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    /** @var \Drupal\commerce_option\Entity\ProductOptionInterface $option */
    $option = $this->entity;
    ...
    $form['id'] = [
      ...
      '#machine_name' => [
        'exists' => '\Drupal\commerce_option\Entity\ProductOption::load',
      ],
      // Option field names are constructed as 'option_' + id, and must
    ...
    $form['elementType'] = [
      ...
      '#description' => $this->t('Controls how the option is displayed on the add to cart form.'),
      '#options' => [
        'radios' => $this->t('Radio buttons'),
        'select' => $this->t('Select list'),
        'commerce_product_rendered_option' => $this->t('Rendered option'),
      ],
    ...
    foreach ($variation_types as $variation_type_id => $variation_type) {
      if (!$option->isNew() && isset($option_field_map[$variation_type_id])) {
        $used_options = array_column($option_field_map[$variation_type_id], 'option_id');
        if (in_array($option->id(), $used_options)) {
          ...
          if (!$this->optionFieldManager->canDeleteField($option, $variation_type_id)) {
    ...
    if ($this->moduleHandler->moduleExists('content_translation')) {
      ...
      if (!$option->isNew()) {
        ...
        $enabled = $translation_manager->isEnabled('commerce_option_option_value', $option->id());
      }
    ...
    // The option acts as a bundle for options values, so the values can't
  ...
  public function buildValuesForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($value_map as $index => $id) {
      ...
      if ($id == '_new') {
        $value = $this->entityTypeManager->getStorage('commerce_option_option_value')->create([
    ...
    $access_handler = $this->entityTypeManager->getAccessControlHandler('commerce_option_option_value');
  ...
  /**
   * Submit callback for resetting option value ordering to alphabetical.
   */
  ...
  protected function saveValues(array $form, FormStateInterface $form_state) {
    ...
    if (!empty($delete_queue)) {
      $value_storage = $this->entityTypeManager->getStorage('commerce_option_option_value');
    ...
    foreach ($form_state->getValue(['values']) as $index => $value_data) {
      ...
      /** @var \Drupal\commerce_option\Entity\ProductOptionValueInterface $value */
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($selected_variation_types) {
      foreach ($selected_variation_types as $selected_variation_type) {
        $this->optionFieldManager->createField($this->entity, $selected_variation_type);
      }
    }
    if ($unselected_variation_types) {
      foreach ($unselected_variation_types as $unselected_variation_type) {
        $this->optionFieldManager->deleteField($this->entity, $unselected_variation_type);
      }
    }
    ...
    if ($this->moduleHandler->moduleExists('content_translation')) {
      ...
      if ($translation_manager->isEnabled('commerce_option_option_value', $this->entity->id()) != $enabled) {
        $translation_manager->setEnabled('commerce_option_option_value', $this->entity->id(), $enabled);
