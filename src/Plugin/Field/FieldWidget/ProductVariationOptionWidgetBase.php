...
/**
 * Provides the base structure for product variation option widgets.
 *
 * Product variation option widget forms depends on the 'product' being present
 * in $form_state.
 *
 * @see \Drupal\commerce_product\Plugin\Field\FieldFormatter\AddToCartFormatter::viewElements().
 */
abstract class ProductVariationOptionWidgetBase extends WidgetBase implements ContainerFactoryPluginInterface {
   ...
   */
  public function __construct($plugin_id, $plugin_definition, FieldDefinitionInterface $field_definition, array $settings, array $third_party_settings, EntityTypeManagerInterface $entity_type_manager, EntityRepositoryInterface $entity_repository, PrivateTempStoreFactory $temp_store_factory) {
  ...
  public function massageFormValues(array $values, array $form, FormStateInterface $form_state) {
    ...
    foreach ($values as $key => $value) {
      $this->tempStore->get('commerce_option')->set('options', $value['options']);
      $values[$key] = [
